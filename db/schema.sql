...
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";
  ...
  CREATE TABLE IF NOT EXISTS container_type_lu (
    label       text NOT NULL,
  ...
  CREATE TABLE IF NOT EXISTS container_size_lu (
    label       text NOT NULL,
    value       text PRIMARY KEY,
    created_at  timestamptz,
    created_by  text,
  ...
  CREATE TABLE IF NOT EXISTS cargo_type_lu (
    label       text NOT NULL,
    value       text PRIMARY KEY,
    created_at  timestamptz,
    created_by  text,
  ...
  CREATE TABLE IF NOT EXISTS package_type_lu (
    label       text NOT NULL,
    value       text PRIMARY KEY,
    created_at  timestamptz,
    created_by  text,
  ...
  CREATE TABLE IF NOT EXISTS activity_lu (
    ...
    modified_by   text,
    is_active     boolean DEFAULT true,
    PRIMARY KEY (activity_type, activity_code)
  );
  ...
  CREATE TABLE IF NOT EXISTS ops_job (
    ...
    enquiry_number      text,
    job_type            text CHECK (job_type IN ('Air','Sea','Land')),
    transport_mode      text,
    service_type        text,
    service_subcategory text,
    parent_job_id       uuid REFERENCES ops_job(id),
    customer_id         uuid, -- Soft FK
    agent_id            uuid, -- Soft FK
    ...
    source_state        text,
    source_country      text,
    branch_id           uuid, -- Soft FK
    branch_name         text, -- Snapshot
    inco_term_code      text, -- Global Lookup (Soft FK)
    commodity           text,
    classification      text,
    sales_executive_id  uuid, -- Soft FK
    sales_executive_name text, -- Snapshot
    operations_exec_id  uuid, -- Soft FK
    operations_exec_name text, -- Snapshot
    cs_executive_id     uuid, -- Soft FK
    cs_executive_name   text, -- Snapshot
  ...
  CREATE TABLE IF NOT EXISTS ops_package (
    ...
    container_no               text,
    container_type             text REFERENCES container_type_lu(value),
    container_size             text REFERENCES container_size_lu(value),
    gross_weight_kg            numeric(10,3),
    net_weight_kg              numeric(10,3),
    volume                     numeric(10,3),
    carrier_seal_no            text,
    commodity_cargo_description text,
    package_type               text REFERENCES package_type_lu(value),
    cargo_type                 text REFERENCES cargo_type_lu(value),
  ...
  CREATE TABLE IF NOT EXISTS ops_party (
    ...
    shipper_id                 uuid, -- Soft FK
    consignee_id               uuid, -- Soft FK
    notify_party_id            uuid, -- Soft FK
    switch_bl_shipper_id       uuid, -- Soft FK
    switch_bl_consignee_id     uuid, -- Soft FK
    switch_bl_notify_party_id  uuid, -- Soft FK
    origin_agent_id            uuid, -- Soft FK
    destination_agent_id       uuid, -- Soft FK
    created_at                 timestamptz DEFAULT now(),
    created_by                 text,
    modified_at                timestamptz,
    modified_by                text,
  ...
  CREATE TABLE IF NOT EXISTS ops_billing (
    ...
    activity_type           text,
    activity_code           text,
    billing_party_id        uuid, -- Soft FK
    FOREIGN KEY (activity_type, activity_code) REFERENCES activity_lu (activity_type, activity_code),
    po_number               text,
    po_date                 timestamptz,
    currency_code           char(3), -- Global Lookup
    quantity                numeric(20,2),
    unit_price              numeric(10,3),
    amount_without_tax      numeric(30,3),
    tax_code                text, -- Global Lookup
    ...
    supporting_doc_url      text[],
    file_region             text,
    amount_primary_currency numeric(20,3),
    created_at              timestamptz DEFAULT now(),
    created_by              text,
    modified_at             timestamptz,
    modified_by             text,
